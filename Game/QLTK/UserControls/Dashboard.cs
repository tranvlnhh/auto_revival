...
namespace QLTK.UserControls
{
    public partial class Dashboard : UserControl
    {
        ...
        Account get_account_intput_batch(int num) => new Account()
        {
            ...
            size = SizeView.Text,
            zoom = int.Parse(ZoomView.Text),
            proxy = get_proxy_intput()
        };
        ...
        bool exist_account(Account acc)
            => get_list_account.Any(a => (a.username == acc.username && a.password == acc.password && a.server == acc.server && a.size == acc.size));
        ...
        async Task Login(Account account)
        {
            if (account.process == null || account.process.HasExited)
            {
                ...
                account.process = Process.Start("123Tool.exe", $"{account.size} {account.zoom} -silent-crashes -nolog -disable-gpu-skinning -releaseCodeOptimization -disableManagedDebugger -accept-apiupdate -no-stereo-rendering");// -batchmode -nographics   $"-screen-width {ss[0]} -screen-height {ss[1]}");
        ...
        private void btnAdd_Click(object sender, EventArgs e)
        {
            ...
            if (string.IsNullOrEmpty(SizeView.Text))
            {
                Utils.notification("Size không thể để trống!", MessageBoxIcon.Error);
                SizeView.Focus();
                return;
            }
            if (string.IsNullOrEmpty(ZoomView.Text))
            {
                Utils.notification("Zoomlevel không thể để trống!", MessageBoxIcon.Error);
                ZoomView.Focus();
                return;
            }
            int zoom;
            if (!int.TryParse(ZoomView.Text, out zoom) || zoom < 1 || zoom > 4)
            {
                Utils.notification("Zoomlevel là một số có giá trị từ 1 đến 4!", MessageBoxIcon.Error);
                ZoomView.Focus();
                return;
            }
            ...
            if (cbProxy.Checked)
            {
                ...
                {
                    Utils.notification("IP không thể để trống!", MessageBoxIcon.Error);
                    ip.Focus();
                    return;
                }
                if (string.IsNullOrEmpty(port.Text))
                {
                    Utils.notification("Port không thể để trống!", MessageBoxIcon.Error);
                    port.Focus();
                    return;
                }
            }
        ...
        private void btnEdit_Click(object sender, EventArgs e)
        {
            ...
            if (list_edit.Count == 1)
            {
                if (Utils.Question($"Bạn có chắc chắn muốn sửa tài khoản {list_edit[0].username}!") == DialogResult.Yes)
                {
                    ...
                    list_edit[0].size = SizeView.Text;
                    list_edit[0].zoom = int.Parse(ZoomView.Text);
                    list_edit[0].proxy = get_proxy_intput();
        ...
        {
            SortTab(get_list_account_opened);
        }
        ...
        private void dataAccount_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            var pr = JsonMapper.ToObject(acc.proxy);
            ip.Text = (string)pr["ip"];
            port.Text = (string)pr["port"];
            username.Text = (string)pr["user"];
            password.Text = (string)pr["pass"];
        }

        private void cbbSize_SelectedValueChanged(object sender, EventArgs e)
        {
            if(cbbSize.SelectedIndex == 0)
            {
                SizeView.Text = "1024x600";
                return;
            }
            SizeView.Text = "240x320";
        }
    }
}
